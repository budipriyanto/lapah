...
export default function EditDestinasi() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from("destination_images").select("*").eq("destination_id", id).order("image_order"),
    ]).then(([destRes, imgRes]) => {
      const d = destRes.data as Destination | null;
      if (!d) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (newImages.length > 0) {
      await supabase.from("destination_images").insert(newImages);
    }
                  ...
                  onClick={async () => {
                    ...
                    input.onchange = async () => {
                      ...
                      try {
                        ...
                        if (json.url) {
                          updateImage(i, "image_url", json.url);
