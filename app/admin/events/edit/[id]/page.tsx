...
export default function EditEvent() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from("event_images").select("*").eq("event_id", id).order("image_order"),
    ]).then(([evtRes, imgRes]) => {
      const evt = evtRes.data as Event | null;
      if (!evt) {
      ...
      const loaded = (imgRes.data ?? []) as EventImage[];
      if (loaded.length > 0) {
        setImages(loaded.map((img, i) => ({
          id: img.id,
          url: img.image_url,
          is_hero: img.is_hero,
          order: i,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const currentIds: string[] = images.filter((img): img is ImageEntry & { id: string } => !!img.id).map((img) => img.id);
    const { data: existingRows } = await supabase.from("event_images").select("id").eq("event_id", id);
    const existingIds = (existingRows ?? []).map((r) => r.id);
    const toDelete = existingIds.filter((eid) => !currentIds.includes(eid));
    if (toDelete.length > 0) {
      await supabase.from("event_images").delete().in("id", toDelete);
    }
    ...
    if (imageRows.length > 0) {
      await supabase.from("event_images").upsert(imageRows);
    }
  ...
  return (
    <div>
      <h1 className="mb-6 text-xl font-bold text-[#1a1a1a]">Edit Event</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Input label="Waktu (opsional)" value={form.time} onChange={(v) => setForm((f) => ({ ...f, time: v }))} />
        </div>
